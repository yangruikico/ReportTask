...
public class TimePickerView extends BasePickerView implements View.OnClickListener {
    ...
    private void initView(Context context) {
        ...
        if (mPickerOptions.customListener == null) {
            ...
            //顶部标题
            TextView tvTitle = (TextView) findViewById(R.id.picker_title);
        ...
        if (tag.equals(TAG_SUBMIT)) {
            if (!isEnd) {
                try {
                    ...
                    long miniTimeL = birthday.getTime();

                    if (selectTime <= miniTimeL) {
                        ViewUtil.Toast("结束时间不能小于开始时间");
                    } else {


                        if (selectTime < System.currentTimeMillis() + 1000) {

                            ViewUtil.Toast("点名时间结束时间不能小于当前时间");
